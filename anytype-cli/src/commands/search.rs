use anytype_core::{AnytypeClient, SearchRequest};
...
async fn search(client: &AnytypeClient, args: SearchArgs) -> Result<()> {
            ...
            if shown_props == 0 {
                for (key, value) in properties.iter().take(2) {
                    let display_value = match value {
                        ...
                        other => serde_json::to_string(&other).unwrap_or_else(|_| "N/A".to_string()),
                    };
    ...
    if shown_end < total {
        println!("💡 Showing results {}-{} of {}. Use --offset {} to see more.", 
            args.offset + 1, shown_end, total, shown_end);
    }
