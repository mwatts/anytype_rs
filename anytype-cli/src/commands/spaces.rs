...
pub async fn handle_spaces_command(args: SpacesArgs) -> Result<()> {
    ...
    match args.command {
        ...
        SpacesCommand::Create { space_id, name, type_key } => create_object(&client, &space_id, &name, &type_key).await,
    }
}
...
async fn list_objects(client: &AnytypeClient, space_id: &str, limit: u32) -> Result<()> {
    ...
    let objects = client.list_objects(space_id).await
    ...
    println!("✅ Found {} objects (showing first {}):", total_objects, display_count);
    
    for object in objects.into_iter().take(display_count) {
        println!("  📄 {} (Space: {})", object.id, 
...
async fn create_object(client: &AnytypeClient, space_id: &str, name: &str, type_key: &str) -> Result<()> {
    ...
    let response = client.create_object(space_id, request).await
        .context("Failed to create object")?;
    
    println!("✅ Object created successfully!");
    println!("   📄 Object ID: {}", response.object.id);
    println!("   🏠 Space ID: {}", response.object.space_id.as_deref().unwrap_or("Unknown"));
    println!("   📝 Name: {}", response.object.name.as_deref().unwrap_or("Unnamed"));
